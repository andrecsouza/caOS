...
import org.cagrid.workflow.workflowmanagerservice.workflowManagerService.WorkflowManagerInstanceDescriptorDocument;
...
public class WorkflowDescriptorParser {
	...
	public static org.cagrid.workflow.manager.descriptor.WorkflowManagerInstanceDescriptor parseWorkflowDescriptor(
		...
		Reader stringReader = new StringReader(xmlWorkflowDescription);
		...
		// TODO Trying to do in a different way 
		/*ByteArrayInputStream is = new ByteArrayInputStream(xmlWorkflowDescription.getBytes());
		XMLDecoder dec = new XMLDecoder(is);
		WorkflowManagerInstanceDescriptor convertedDescriptor = (WorkflowManagerInstanceDescriptor) dec.readObject(); // */
	...
	private static WorkflowManagerInstanceDescriptor convertFromXMLBeans(
		...
		if( managerInstanceDescriptor != null ){
			...
			converted.setLocalWorkflows(extractLocalWorkflows(managerInstanceDescriptor.getLocalWorkflows()));
		}
	...
	private static org.cagrid.workflow.manager.descriptor.WorkflowPortionsDescriptor extractLocalWorkflows(
			WorkflowPortionsDescriptor localWorkflows) throws MalformedURIException {
		...
		if( localWorkflows != null ){
			...
			converted.setLocalWorkflowDesc(extractWorkflowParts(localWorkflows.getLocalWorkflowDescArray()));
		}
	...
	private static WorkflowPortionDescriptor[] extractWorkflowParts(
			org.cagrid.workflow.workflowmanagerservice.workflowManagerService.WorkflowPortionDescriptor[] workflowPartsArray) throws MalformedURIException {
	...
	private static WorkflowStageDescriptor[] extractInvocationHelperDescs(
			org.cagrid.workflow.workflowmanagerservice.workflowManagerService.WorkflowStageDescriptor[] invocationHelperDescsArray) throws MalformedURIException {
	...
	private static EndpointReferenceType extractEPR(
		...
		if( (proxyEPR != null) && (proxyEPR.getAddress() != null) ){
			...
			converted = new EndpointReferenceType(new Address(address.getStringValue()));  // Address URI
			
			ReferencePropertiesType props = proxyEPR.getReferenceProperties();   // Reference properties
			org.apache.axis.message.addressing.ReferencePropertiesType convertedProps = new 
			    org.apache.axis.message.addressing.ReferencePropertiesType();
			
			String properties = props.toString();
			
			
			System.out.println("[extractEPR] EPR properties: "+ properties);  //DEBUG
			
			convertedProps.add(properties);
			converted.setProperties(convertedProps );
